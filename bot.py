...
def get_meme(subreddit_name="memes"):
    try:
        ...
        print(f"Error fetching meme from subreddit '{subreddit_name}': {e}")
        return None, None
...
class RoleChecker:
    def __init__(self, role_name, server_id, role_id):
...
async def help_command(interaction: discord.Interaction):
    ...
    if await checker.check_role(interaction):  # Check for the required role
        ...
        await interaction.followup.send(embed=help_embed, view=view)
    else:
...
async def memes_by_number(interaction: discord.Interaction, count: int):
    ...
    if await checker.check_role(interaction):
        ...
        try:
            # Fetch memes from a subreddit (e.g., "memes")
            reddit = asyncpraw.Reddit(client_id='YOUR_CLIENT_ID', client_secret='YOUR_CLIENT_SECRET', user_agent='YOUR_USER_AGENT')
            subreddit_instance = await reddit.subreddit("memes")
            memes = []
            
            async for submission in subreddit_instance.new(limit=count):
                memes.append({
...
async def setchannel(interaction: discord.Interaction, channel: discord.TextChannel, search_query: str, interval: str):
    ...
    if await checker.check_role(interaction):
        try:
            ...
            # Store the channel and search query in active_channels
